...
from keras.layers import Input, concatenate, Conv2D, MaxPooling2D, UpSampling2D, Reshape, core, \
    Dropout,convolutional,Conv2DTranspose,Concatenate,Activation
...
def side_branch(x, factor):
    ...
    x = Conv2DTranspose(1, kernel_size, strides=factor, padding='same', data_format='channels_first',use_bias=False, activation=None)(x)
...
def side_branch2(x, factor):
    ...
    kernel_size = (2 * factor, 2 * factor)
    x = Conv2DTranspose(2, kernel_size, strides=factor, padding='same', data_format='channels_first',use_bias=False, activation=None)(x)
...
def get_unet2(n_ch,patch_height,patch_width):
    ...
    fuse = Activation(activ, name='ofuse')(fuse)
    #
    # # model
...
def get_unet4(n_ch,patch_height,patch_width):
    ...
    loss = 'categorical_crossentropy'
    model.compile(optimizer='sgd', loss={'oofuse': loss,},metrics=['accuracy'])
...
def get_hed(n_ch,patch_height,patch_width):
    ...
    # Block 1
    x = Conv2D(64, (3, 3), activation='relu', padding='same',  data_format='channels_first',name='block1_conv1')(img_input)
    ...
    x = MaxPooling2D((2, 2), strides=(2, 2), padding='same',  data_format='channels_first',name='block1_pool')(x) # 240 240 64
    ...
    x = MaxPooling2D((2, 2), strides=(2, 2), padding='same', data_format='channels_first', name='block2_pool')(x) # 120 120 128
    print(x.name, x.shape)
    ...
    x = MaxPooling2D((2, 2), strides=(2, 2), padding='same',  data_format='channels_first',name='block3_pool')(x) # 60 60 256
    print(x.name, x.shape)
    ...
    x = MaxPooling2D((2, 2), strides=(2, 2), padding='same',  data_format='channels_first',name='block4_pool')(x) # 30 30 512
    print(x.name, x.shape)
    ...
    print(x.name, x.shape,end="  ")
    x = Conv2D(512, (3, 3), activation='relu', padding='same',  data_format='channels_first',name='block5_conv3')(x) # 30 30 512
    print(x.name, x.shape)
    b5= side_branch(x, 16) # 480 480 1

    # fuse
    fuse = Concatenate(axis=1)([b1, b2, b3, b4, b5])
    fuse = Conv2D(1, (1,1), padding='same', use_bias=False, activation=None, data_format='channels_first')(fuse) # 480 480 1
