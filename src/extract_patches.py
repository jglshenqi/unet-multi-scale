...
def get_data_training(DRIVE_train_imgs_original,
    ...
    patches_imgs_train, patches_masks_train = extract_random(train_imgs, train_masks, patch_height, patch_width,
                                                             N_subimgs, train_coordinate, inside_FOV)
...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height,
    ...
    test_masks = test_masks[number:number+1,:,:,:]
...
def extract_random(full_imgs,full_masks, patch_h, patch_w, N_patches, train_coordinate, inside=True):
    ...
    patches = np.empty((training_shape[0]*training_shape[1],full_imgs.shape[1],patch_h,patch_w))
    patches_masks = np.empty((training_shape[0]*training_shape[1],full_masks.shape[1],patch_h,patch_w))
    img_h = full_imgs.shape[2]  #height of the full image
    img_w = full_imgs.shape[3] #width of the full image
    # (0,0) in the center of the image
    patch_per_img = int(training_shape[0]*training_shape[1]/full_imgs.shape[0])  #N_patches equally divided in the full images
    ...
    for i in range(training_shape[0]):  #loop over the full images
        ...
        while k < training_shape[1]:
            ...
            k+=1  #per full_img
        # list.append(list_)
    # save_node(list,"./DRIVE_datasets_training_testing/train_patch200.pickle")
    # print("end")
    # i = input()
    return patches, patches_masks

def extract_random2(full_imgs,full_masks, patch_h, patch_w, N_patches, train_coordinate, inside=True):
    ...
    for i in range(full_imgs.shape[0]):  # loop over the full images
        ...
        while k < patch_per_img:
            ...
            k += 1  # per full_img
        list.append(list_)
    save_node(list, "./STARE_datasets_training_testing/train_patch200.pickle")
    print ("stop")
    i = input()
    return patches, patches_masks
...
def is_patch_inside_FOV(x,y,img_w,img_h,patch_h):
    ...
    R_inside = 1168 - int(patch_h * np.sqrt(2.0) / 2.0) #radius is 270 (from DRIVE db docs), minus the patch diagonal (assumed it is a square #this is the limit to contain the full patch in the FOV
