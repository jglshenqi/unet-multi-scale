...
def extract_random_in(train_mask, patch_h, patch_w, N_patches):
    ...
    for i in range(train_mask.shape[0]):  # loop over the full images
        ...
        list.append(list_)
    print(zero, one)
    save_node(list, "./DRIVE_datasets_training_testing/train_patch_inside.pickle".replace("DRIVE", dataset))
    print("end")
...
get_data_training(DRIVE_train_mask=path_data + config.get('data paths', 'train_border_masks').replace("DRIVE", dataset),
